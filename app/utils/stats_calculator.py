...
class SpotifyStatsCalculator:
    ...
    def _calculate_skip_stats(self):
        ...
        total_plays = db_session.query(func.count(StreamingHistory.id)).filter(
            StreamingHistory.username == self.username
        ).scalar() or 0
        
        skipped_tracks = db_session.query(func.count(StreamingHistory.id)).filter(
            StreamingHistory.username == self.username,
            StreamingHistory.skipped == True
        ).scalar() or 0
    ...
    def _calculate_listening_by_weekday(self):
        ...
        weekday_stats = db_session.query(
    ...
    def _save_stats_to_db(self, stats):
        ...
        db_session.query(UserStats).filter(UserStats.username == self.username).delete()
