...
def bar_plot_df(df, x, y, title=None, x_label=None, y_label=None, sort=False):
...
class musicHistory:
    ...
    def load_over_time(self):
        ...
        bar_plot_df(df_time_sum, 'endTime', 'hrPlayed', title=f"Listening time to Spotify streams per day: {self.start_date}/{self.end_date}", y_label='Hours [h]' )
    ...
    def toptracks(self, top=50):
        ...
        bar_plot_df(df_top, 'fullName', 'streamTimeHr', title=f"Listening time to Spotify streams by track: {self.start_date}/{self.end_date}", x_label='Track name', y_label='Stream time[Hr]', sort=True)
        bar_plot_df(df_top, 'fullName', 'noStreams', title=f"Number of streams by track: {self.start_date}/{self.end_date}", x_label='Track name', y_label='Number of streams', sort=True)
